...
main_keyboard = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="📢 Новости", url=CHANNEL_URL)],
])
...
def buy_product_keyboard(product_id, price: int = 0, gift: bool = True):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="🎁 Подарить другу", callback_data=f"gift_{product_id}"),
        ],
        [InlineKeyboardButton(text="◀️ В каталог", callback_data="buy")],
    ])
    return kb
...
def deposit_methods_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💎 Crypto Bot · USDT/TON", callback_data="pay_method_crypto"),
         InlineKeyboardButton(text="⭐ Telegram Stars", callback_data="pay_method_gifts")],
        [InlineKeyboardButton(text="💵 Рубли · перевод", callback_data="pay_method_rubles"),
         InlineKeyboardButton(text="💱 Перевести пользователю", callback_data="transfer_balance")],
...
def account_actions_keyboard(phone_clean: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Запросить код", callback_data=f"request_code_{phone_clean}"),
...
def subscribe_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⏳ Отложить", callback_data="subscribe_later")],
    ])
